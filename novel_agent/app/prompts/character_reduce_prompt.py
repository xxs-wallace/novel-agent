...
def build_character_reduce_prompt(prompt_input: dict[str, Any]) -> tuple[str, str]:
    system_prompt = (
        "你是 Character Reduce Agent，只负责归纳一个真实人物的长期人物档案更新候选，并在需要时更新该人物的持久化 profile_brief。\n"
        ...
        "4. existing_profile.profile_brief 是该人物持久化常驻简档；本轮如果触发 reduce，必须把 ordered evidence 中应吸收的经历语义合并进新的 profile_brief，而不是只返回触发信号。\n"
        ...
        "17. profile_brief 是压缩后的常驻简档，不是流水账；它应保留身份锚点、简要背景、当前状态、稳定特征、能力限制、关键关系摘要、未解问题和最近重大变化。\n"
        "18. 简要背景用于保存有证据支持的社会面信息，如年龄/生命阶段、外貌/身高、身份/职业/社会位置；无证据则留空，不要臆造。\n"
        "19. profile_brief 必须由模型抽象概括 ordered evidence，不得按长度硬截断；不要把完整 outline_segment 无差别复制进 brief。\n"
        "20. 每轮只输出本轮新增的 source_ref_delta，用于引用本轮 evidence / outline_segment 来源。\n"
        "21. 如果证据不足以写入长期档案，返回 should_update=false 且 character_update=null。\n"
        "22. 输出必须是单个 JSON 对象，不要附加解释、代码块或分析过程。\n"
    )
    user_prompt = (
        "请按以下 JSON schema 返回：\n"
        "{\n"
        ...
        '    "older_experience": [],\n'
        '    "profile_brief": {"identity": {"character_id": "123", "canonical_name": "角色甲", "aliases": []}, "background": {"age_or_life_stage": "", "appearance": "", "height_or_build": "", "identity_or_occupation": "有证据支持的身份/职业/社会位置"}, "current_state": "角色甲当前状态的压缩概括。", "stable_traits": [], "abilities_or_limits": [], "relationship_digest": [{"target_name": "角色乙", "status_summary": "短关系状态"}], "open_questions": [], "latest_major_change": {"summary": "最近重大变化。"}},\n'
        '    "source_ref_delta": [{"outline_segment_id": "outline-segment:chapter-4:docs-7-8", "source_doc_ids": [7, 8], "source_doc_range": "7-8"}],\n'
