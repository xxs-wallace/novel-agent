...
def build_chapter_outline_segment_prompt(prompt_input: dict[str, Any]) -> tuple[str, str]:
    system_prompt = (
        ...
        "4. 高显著事件优先保留：导致人物性格、立场、能力或身体状况发生显著变化的事件；导致人物关系发生重大变化的事件；对人物周围的人际关系、组织、社会或世界规则造成显著影响的事件；揭示重大悬念的事件；颠覆或改写读者对过往剧情理解的事件。\n"
        "5. 普通章节目标长度约 180-320 个中文字符；若 summary_md 同时包含多个高显著事件或事件因果密度很高，可扩展到 320-520 个中文字符，但仍必须明显短于 summary_md。\n"
        "6. 不要为了满足长度而删除高显著事件；不要机械截断，不要以省略号结尾，不要复制原文句子或露骨细节。\n"
        "7. 输出必须是单个 JSON 对象。\n"
    )
    user_prompt = (
        "请按以下 JSON schema 返回：\n"
